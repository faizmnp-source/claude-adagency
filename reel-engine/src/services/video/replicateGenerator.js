...
const MODELS = {
  ...
  veo2_flash: {
    ...
    input: (imageUrl, prompt, duration, extendFromUrl) => ({
      prompt: prompt || 'product showcase, smooth camera motion, professional commercial',
      image: !extendFromUrl ? imageUrl : undefined,
      video: extendFromUrl || undefined,
      aspect_ratio: '9:16',
      duration: 5,
    }),
  },
  ...
  wav2lip: {
    owner: 'cjwbw', name: 'wav2lip',
    label: '👄 Lip Sync (Wav2Lip)', resolution: '720p', clipDuration: 0, estimatedUsdPerClip: 0.15,
...
export async function generateSceneClip({ imageUrl, prompt, durationSeconds, sceneNumber, reelId, modelKey, packageId, quality, extendFromUrl }) {
  ...
  logger.info('Generating scene clip', { reelId, sceneNumber, model: `${model.owner}/${model.name}`, resolvedKey, extending: !!extendFromUrl });
...
/**
 * Lip sync a video with an audio file
 */
export async function lipSyncVideo({ videoUrl, audioUrl }) {
  const model = MODELS.wav2lip;
  logger.info('Starting Lip Sync', { videoUrl: videoUrl.substring(0, 40), audioUrl: audioUrl.substring(0, 40) });

  const input = model.input(videoUrl, audioUrl);
  const prediction = await runPrediction(model, input);

  const syncedUrl = Array.isArray(prediction.output) ? prediction.output[0] : prediction.output;
  if (!syncedUrl) throw new Error('Lip sync failed: No output URL');
