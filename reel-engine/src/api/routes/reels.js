...
router.post('/:id/generate-video', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < clipPlan.length; i++) {
      ...
      const resolvedModelKey = packageId === 'ultra' ? 'veo2' : (packageId === 'creator' ? 'wan720p' : (packageId === 'viral' ? 'luma' : (packageId === 'starter' ? 'wan480p' : modelKey)));
      ...
      const clip = await generateSceneClip({
        ...
        modelKey,
        quality,
        extendFromUrl: (resolvedModelKey === 'veo2' || resolvedModelKey === 'veo2_flash') ? lastClipUrl : null,
      });
      clips.push(clip);
      lastClipUrl = clip.videoUrl;
    }
...
router.post('/:id/stitch', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const { VIDEO_PACKAGES, lipSyncVideo } = await import('../../services/video/replicateGenerator.js');
    const pkg = reel.package ? VIDEO_PACKAGES[reel.package] : null;
    const wantVoice = pkg?.voice ?? false;
    const wantMusic = pkg?.music ?? false;
    const wantLipSync = req.body.lipSync ?? (reel.package === 'ultra' || reel.package === 'viral');

    logger.info('Stitching clips', { reelId, clipCount: clips.length, package: reel.package, wantVoice, wantMusic, wantLipSync });
    ...
    const { uploadToS3, getPresignedUrl } = await import('../../services/storage/s3.js');
    ...
    if (wantVoice && process.env.REPLICATE_API_TOKEN) {
      try {
        ...
        if (script) {
          ...
          if (wantLipSync) {
            // ── LIP SYNC STEP ──────────────────────────────────────────
            logger.info('Starting Lip Sync process', { reelId });
            audioMessages.push('⏳ Starting Lip Sync (Wav2Lip)...');

            // Upload merged silent video and voice to S3 so Replicate can access them
            const videoS3Key = `temp/${reelId}/silent-merged.mp4`;
            const voiceS3Key = `temp/${reelId}/voiceover.mp3`;

            const videoUrl = await uploadToS3(currentVideoPath, videoS3Key, 'video/mp4');
            const voiceUrl = await uploadToS3(voicePath, voiceS3Key, 'audio/mpeg');

            const syncedUrl = await lipSyncVideo({ videoUrl, audioUrl: voiceUrl });
            const syncedPath = tmpFile(`synced-${reelId}.mp4`);
            await downloadFile(syncedUrl, syncedPath);
            tempFiles.push(syncedPath);

            currentVideoPath = syncedPath;
            audioMessages.push('✅ Lip Sync complete (Wav2Lip)');
          } else {
        ...
        logger.warn('ElevenLabs voiceover or LipSync failed (non-fatal)', { reelId, err: voiceErr.message });
        audioMessages.push(`⚠️ Voice/LipSync skipped: ${voiceErr.message}`);
      }
