...
def check_and_merge(project_id: str):
    ...
    content = supabase.table("content").select("voice_url, video_url, script, final_url") \
        .eq("project_id", project_id).single().execute().data

    if not content["voice_url"] or not content["video_url"]:
        return  # wait for the other job
    ...
    try:
        ...
        import threading
        threading.Thread(target=_merge_run, args=(project_id,), daemon=True).start()


def _merge_run(project_id: str):
    """Core merge logic — callable directly without Celery."""
    supabase = get_supabase()
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(output_path, "rb") as f:
                supabase.storage.from_("assets").upload(f"final/{project_id}.mp4", f.read(), {"content-type": "video/mp4"})
        ...
        try:
            ...
            import threading
            threading.Thread(target=send_preview, args=(project_id,), daemon=True).start()
