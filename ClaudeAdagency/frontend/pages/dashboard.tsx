...
const STATUS_META: Record<string, { label: string; color: string; dot: string }> = {
  ...
  posted:     { label: "Posted",     color: "bg-lime/10 text-lime border-lime/20",                  dot: "bg-lime" },
    ...
    <div className="relative min-h-screen z-10 bg-ink">
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="mb-8 fade-in">
          <h1 className="display text-3xl text-white mb-1">YOUR PROJECTS</h1>
        ...
        {projects.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8 fade-in">
            {[
              ...
              { label: "Posted", value: counts.posted || 0, color: "text-lime" },
            ].map(s => (
              <div key={s.label} className="glass px-5 py-4">
                <p className={`display text-2xl ${s.color}`}>{s.value}</p>
              ...
              <button key={f} onClick={() => setFilter(f)}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all duration-200
                  ${filter === f
                    ...
                    : "bg-white/5 border-white/10 text-white/40 hover:border-lime/30 hover:text-white"}`}>
        ...
        {!loading && projects.length === 0 && (
          <div className="glass text-center py-20 fade-in">
            <div className="w-16 h-16 rounded-sm bg-lime/20 border border-lime/30 mx-auto mb-4 flex items-center justify-center">
            ...
            <h2 className="display text-2xl text-white mb-2">NO REELS YET</h2>
            <p className="text-white/35 text-sm mb-6">Create your first AI-powered Instagram Reel</p>
            <Link href="/" className="lime-btn px-6 py-3 inline-block">Create First Reel</Link>
          </div>
        )}
